...
function submitMessage() {
  ...
  window.setTimeout(() => { mode.value = 'idle'; }, 720);
}
...
<template>
  <form class="agent-dock" @submit.prevent="mode === 'composing' ? submitMessage() : openComposer()">
    <div class="agent-dock__composer" :class="{ 'is-open': mode === 'composing' }">
      ...
      <textarea ref="textarea" v-model="message" aria-label="向 Pathlight 提问" placeholder="粘贴招聘方的问题，或说说你想判断什么…" @keydown="handleKeydown" />
    ...
    <div class="agent-dock__bar">
      ...
      <div class="agent-dock__actions">
        ...
        <button class="agent-dock__send" type="submit" :aria-label="mode === 'composing' ? '发送消息' : '打开对话输入框'">
